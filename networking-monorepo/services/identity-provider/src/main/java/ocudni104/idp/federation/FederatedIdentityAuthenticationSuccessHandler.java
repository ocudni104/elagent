...
public class FederatedIdentityAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(
    ...
    if (authentication instanceof OAuth2AuthenticationToken oauthToken) {
      ...
      var user =
          findOrCreateUserFromFederatedLoginUseCase.execute(
              new FindOrCreateUserFromFederatedLoginCommand(email, "google", subject));
      ...
      String deviceOs =
          readAuthorizationRequestAttribute(
      ...
      ResponseCookie sidCookie =
          ResponseCookie.from(sessionCookieName, session.id().value().toString())
              .httpOnly(true)
              .path("/")
              .sameSite("Lax")
              .secure(request.isSecure())
              .maxAge(
                  session.absoluteExpiresAt().getEpochSecond()
                      - session.createdAt().getEpochSecond())
              .build();
  ...
  private static String readAuthorizationRequestAttribute(
      OAuth2AuthorizationRequest request, String name) {
    if (request == null) {
      return null;
    }
